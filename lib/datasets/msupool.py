...
import subprocess
import uuid
from msu_eval import msu_eval
from fast_rcnn.config import cfg

class msupool(imdb):
    def __init__(self, image_set):
        imdb.__init__(self, "msupool")
        self._image_set = image_set 
        self._data_path = os.path.join(self._get_default_path())
        self._classes = ('__background__', # always index 0
                         'qual_gate', 'buoy', 'channel', 'bin', 'bin_cover', 'torpedo_target', 'torpedo_hole', 'torpedo_cover', 'object_pickup', 'object_dropoff', 'path_marker')
    ...
    def image_path_from_index(self, index):
        ...
        for ext in self._image_ext: 
            #image_path = self.shellquote(os.path.join(self._data_path, 'Images', index + ext))
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'MSUPool')
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        for i, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            filename = self._get_voc_results_file_template().format(cls)
            rec, prec, ap = msu_eval(
                filename, annopath, imagesetfile, cls, cachedir, ovthresh=0.5)
            aps += [ap]
            #print('Recall for {} = {:s}'.format(cls, rec))
...
if __name__ == '__main__':
    from datasets.msupool import msupool 
    d = msupool('train')
    res = d.roidb
